    ...
    const usersController = {
      ...
      //! NEW: Generate PDF Report from HTML content received from frontend
      generatePdfReport: asyncHandler(async (req, res) => {
        console.log("DEBUG: generatePdfReport controller started."); // ! NEW DEBUG LOG
        const { htmlContent } = req.body;
        const userId = req.user;
        const user = await User.findById(userId);

        if (!user) {
          console.error("ERROR: User not found for PDF generation."); // ! NEW DEBUG LOG
          res.status(404);
          throw new Error("User not found.");
        }

        if (!htmlContent) {
          console.error("ERROR: No HTML content provided for PDF generation."); // ! NEW DEBUG LOG
          res.status(400);
        ...
        let browser;
        try {
          console.log("DEBUG: Launching Puppeteer browser..."); // ! NEW DEBUG LOG
          browser = await puppeteer.launch({
            headless: true,
            args: ['--no-sandbox', '--disable-setuid-sandbox']
          });
          const page = await browser.newPage();
          console.log("DEBUG: Puppeteer browser launched, new page created."); // ! NEW DEBUG LOG

          await page.setContent(htmlContent, {
            waitUntil: 'networkidle0'
          });
          console.log("DEBUG: Page content set, waiting for network idle."); // ! NEW DEBUG LOG
          ...
          console.log("DEBUG: PDF buffer generated."); // ! NEW DEBUG LOG

          res.setHeader('Content-Type', 'application/pdf');
          res.setHeader('Content-Disposition', `attachment; filename=financial-dashboard-report-${Date.now()}.pdf`);
          res.send(pdfBuffer);
          console.log("DEBUG: PDF sent successfully."); // ! NEW DEBUG LOG

        } catch (error) {
          console.error("ERROR in generatePdfReport (Puppeteer):", error); // ! NEW DEBUG LOG
          if (browser) {
            console.log("DEBUG: Closing browser due to error."); // ! NEW DEBUG LOG
            await browser.close();
          }
          res.status(500);
          throw new Error("Failed to generate PDF report on the server."); // More generic error for frontend
        } finally {
          if (browser) {
            console.log("DEBUG: Ensuring browser is closed in finally block."); // ! NEW DEBUG LOG
            await browser.close();
          }
        }
      }),
