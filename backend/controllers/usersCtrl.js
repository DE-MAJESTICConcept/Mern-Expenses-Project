...
const usersController = {
  ...
  generatePdfReport: asyncHandler(async (req, res) => {
    const { htmlContent } = req.body; // HTML content sent from the frontend
    const userId = req.user; // Get user ID from authentication
    const user = await User.findById(userId);
    ...
    try {
      ...
      // `args: ['--no-sandbox']` is often needed for Puppeteer on Linux environments like Render
      ...
      // Set the HTML content received from the frontend
